/**
 * Leela Chess Zero Engine Service
...
export class LeelaService extends EventEmitter implements OnModuleDestroy {
  ...
  constructor(private configService: ConfigService) {
    const leelaPath = configService.get('LEELA_PATH', '/usr/games/lc0');
    const threads = parseInt(configService.get('LEELA_THREADS', '2'));
  ...
  async initialize(): Promise<void> {
    ...
    this.logger.log('Initializing Leela Chess Zero...');
    ...
    try {
      ...
      if (!this.process.stdin || !this.process.stdout) {
        throw new Error('Failed to create Leela process streams');
      }
      ...
      this.process.stderr?.on('data', (data: Buffer) => {
        this.logger.error(`Leela stderr: ${data.toString()}`);
      });

      this.process.on('exit', (code) => {
        this.logger.warn(`Leela process exited with code ${code}`);
      ...
      this.logger.log('Leela Chess Zero initialized successfully');
    } catch (error) {
      this.logger.error('Failed to initialize Leela', error);
      throw error;
    }
  }
  ...
  async getBestMove(fen: string, options?: EngineOptions): Promise<AnalysisResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        this.stop();
        reject(new Error('Leela analysis timeout'));
  ...
  private async sendCommand(command: string): Promise<void> {
    if (!this.process?.stdin) {
      throw new Error('Leela process not initialized');
    }

    this.logger.debug(`Leela > ${command}`);
    this.process.stdin.write(`${command}\n`);
  }

  private handleOutput(line: string): void {
    this.logger.debug(`Leela < ${line}`);
